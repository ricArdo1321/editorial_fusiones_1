  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-40 flex items-center justify-between px-6 py-4 border-b border-white/10 backdrop-blur-md bg-background/70">
        <Link href={PageRoute.HOME} className="text-xl font-black tracking-tighter uppercase font-sans hover:text-brand transition-colors">
      ...
      {/* Mobile/Overlay Menu */}
      <div
        className={`fixed inset-0 z-30 bg-background flex flex-col items-center justify-center transition-transform duration-500 ease-in-out ${isOpen ? 'translate-y-0' : '-translate-y-full'
          }`}
      >
        <nav className="flex flex-col gap-8 text-center">
          {NAV_LINKS.map((link) => (
            <Link
              ...
              className={`text-4xl md:text-6xl font-black tracking-tight font-sans hover:text-brand transition-colors ${pathname === link.path ? 'text-brand' : 'text-foreground'
                }`}
            >
              {link.label}
            </Link>
